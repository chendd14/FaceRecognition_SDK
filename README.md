...
### SDK授权
SDK采用软授权，执行get_pc_signature.exe，可以获取机器码。调用SetLicenseFile函数，可以设置授权文件的名称。
...
namespace vision {
	...
	public:
		...
		/* 初始化（已采用默认参数初始化，一般情况无需调用此接口），输入参数文件param_path和所要使用的GPU编号device_id */
...
void CameraTest()
{
    ...
    while (true)
    {
        ...
        if (!img.empty()) {
            ...
            face_rects = face_detect->GetFaces(img, key_pts); // 获取人脸框位置同时获取每张脸的9个关键点
...
namespace vision {
    ...
    public:
        ...
        /* 获取比对相似度Top N的比对结果 */
        virtual std::vector<VisionFaceCompareResult> GetResult(const std::vector<float>& feature, int top_k = 1) = 0;
...
namespace vision {
    ...
    public:
        ...
        /* 初始化（已使用默认参数初始化，一般不需要重新初始化），输入参数文件param_path和所要使用的GPU编号device_id */
